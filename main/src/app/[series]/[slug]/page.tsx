  ...
  return (
    <div className="max-w-6xl mx-auto py-12 flex gap-12 justify-center items-start">
      <aside className="w-60 shrink-0 hidden md:block text-left">
        ...
        <div className="font-bold mb-2 text-white">{series}</div>
          ...
          {articles.map((a) => {
            ...
            return (
              <Link
                key={a.slug}
                href={`/${series}/${a.slug}`}
                className={`block px-3 py-2 rounded-lg border border-zinc-800 group transition-colors hover:border-yellow-400 ${a.slug === slug ? "bg-zinc-800 text-yellow-400 border-yellow-400" : "text-zinc-200"}`}
              >
      ...
      <div className="flex-1 min-w-0 flex flex-col katex">
        <div className="w-full max-w-2xl">
          <Link href={`/${series}`} className="text-zinc-400 hover:text-yellow-400 text-sm mb-4 inline-block">← Back to series</Link>
